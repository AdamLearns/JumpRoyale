...
public partial class Arena : Node2D
{
    ...
    private void OnGameTimerDone()
    {
        ...
        _gameDone = true;
        _gameEndTime = DateTime.Now.Ticks;
    ...
    private void ShowEndScreen(string[] winners)
    {
        ...
        for (int i = 0; i < winners.Length; i++)
        {
            ...
            text +=
                $"\t{i + 1}: {playerData.Name}. Height reached: {height}. Games played: {playerData.NumPlays}. Wins: {playerData.Num1stPlaceWins}/{playerData.Num2ndPlaceWins}/{playerData.Num3rdPlaceWins}. Lifetime height: {playerData.TotalHeightAchieved}\n";
        }
    ...
    private void HandleJump(MessageEventArgs e)
    {
        ...
        if (_gameEndTime > 0 && (DateTime.Now.Ticks - _gameEndTime) / TimeSpan.TicksPerMillisecond < 5000)
        {
            return;
        }
